...
<!-- Sidebar -->
<aside class="fixed inset-y-0 left-0 z-50 bg-white border-r border-gray-200 transition-all" :class="{
    ...
    <div class="flex flex-col h-full px-3 pt-3 pb-4 overflow-y-auto">
        ...
        <div class="flex items-center justify-between mb-4">
            <a href="{{ route(auth()->user()?->isKurir() ? 'kurir.dashboard' : 'admin.dashboard') }}"
                class="flex items-center gap-2">
                <img src="{{ asset('assets/images/logo.png') }}" class="h-8 w-8 object-contain" />
                <span class="font-extrabold text-gray-800" x-show="!$store.layout.sidebarCollapsed">FishyGO</span>
            </a>

            <div class="flex items-center gap-1">
                <!-- Collapse (Desktop) -->
                <button @click="$store.layout.sidebarCollapsed = !$store.layout.sidebarCollapsed"
                    class="hidden md:inline-flex text-gray-600 hover:text-gray-900"
                    :title="$store.layout.sidebarCollapsed ? 'Perluas' : 'Ciutkan'">
        ...
        <nav class="flex-1">
            ...
            @can('access-admin')
            <div x-show="!$store.layout.sidebarCollapsed" x-cloak
                class="px-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider mt-2 mb-2">
                Utama
            </div>
            ...
            <div x-data="{ open: false }" class="mt-4">
                <button @click="open = !open" class="group flex items-center justify-between w-full px-3 py-2 rounded-md text-[15px]
                    ...
                    <span x-show="!$store.layout.sidebarCollapsed"
                        class="material-symbols-outlined text-[20px] text-gray-500 transition-transform duration-200"
                ...
                <div x-show="open && !$store.layout.sidebarCollapsed" x-transition class="ml-8 mt-1 space-y-1">
                    <a href="{{ route('admin.kategori.index') }}"
                        class="flex items-center gap-3 px-3 py-1.5 rounded-md text-[14px] transition-colors
                              {{ request()->routeIs('admin.kategori.*') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100' }}">
                        <span class="material-symbols-outlined text-[20px] text-gray-500">category</span>
                        <span>Kategori</span>
                    </a>

                    <a href="{{ route('admin.jenis-ikan.index') }}"
                        class="flex items-center gap-3 px-3 py-1.5 rounded-md text-[14px] transition-colors
                              {{ request()->routeIs('admin.jenis-ikan.*') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100' }}">
                        <span class="material-symbols-outlined text-[20px] text-gray-500">set_meal</span>
                        <span>Jenis Ikan</span>
                    </a>

                    <a href="{{ route('admin.produk.index') }}"
                        class="flex items-center gap-3 px-3 py-1.5 rounded-md text-[14px] transition-colors
                              {{ request()->routeIs('admin.produk.*') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100' }}">
                        <span class="material-symbols-outlined text-[20px] text-gray-500">inventory_2</span>
            ...
            <!-- TRANSAKSI -->
            <div x-show="!$store.layout.sidebarCollapsed" x-cloak
            class="px-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider mt-4 mb-2">
                Transaksi
            </div>

            @php($active = request()->routeIs('admin.pesanan.*'))
            <a href="{{ route('admin.pesanan.index') }}" class="group flex items-center gap-3 px-3 py-2 rounded-md text-[15px] transition-colors
                      {{ $active ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-100' }}"
                :class="{ 'justify-center': $store.layout.sidebarCollapsed }">

                <span class="relative material-symbols-outlined text-[20px]
                             {{ $active ? 'text-indigo-600' : 'text-gray-500 group-hover:text-gray-700' }}">
                    shopping_cart
                </span>
        ...
        <div class="border-t border-gray-200 pt-4 mt-4 space-y-1">
            ...
            <x-alert-confirmation modal-id="confirm-logout-sidebar" title="Keluar akun?"
                message="Anda akan keluar dari sesi saat ini." confirm-text="Keluar" cancel-text="Batal"
                variant="danger" action="{{ route('logout') }}" method="POST">

                <span class="group flex items-center gap-3 px-3 py-2 rounded-md text-[15px]
                             text-gray-700 hover:bg-gray-100 transition-colors w-full cursor-pointer">
                    <span class="material-symbols-outlined text-[20px] text-gray-500 group-hover:text-gray-700">
                        logout
                    </span>
                    <span>Keluar</span>

...
@section('content')
    ...
    <div class="max-w-6xl mx-auto px-4 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div x-data="{ i: {{ (int) $primaryIndex }}, imgs: [] }" x-init="
                    imgs = JSON.parse($refs.imgs.textContent);
                ">
                <script type="application/json" x-ref="imgs">
                    {!! json_encode($photos, JSON_UNESCAPED_SLASHES) !!}
                </script>

                {{-- Foto Utama --}}
                <div class="relative w-full h-80 md:h-96 overflow-hidden rounded-lg shadow-md">
                    <img :src="imgs[i]" alt="{{ $produk->nama_produk }}" class="absolute inset-0 w-full h-full object-cover"
                        loading="lazy">

                    <button @click="i = (i - 1 + imgs.length) % imgs.length"
                        class="btn btn-circle btn-sm absolute left-2 top-1/2 -translate-y-1/2">‹</button>

                    <button @click="i = (i + 1) % imgs.length"
                        class="btn btn-circle btn-sm absolute right-2 top-1/2 -translate-y-1/2">›</button>
                </div>

                {{-- Thumbnail --}}
                <div class="mt-3 flex gap-2 overflow-x-auto pb-1">
                    <template x-for="(src, idx) in imgs" :key="idx">
                        <button @click="i = idx" :class="[
                                'h-16 w-16 rounded overflow-hidden border',
                                idx === i ? 'border-primary' : 'border-base-200'
                            ]">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Kategori:<span class="font-medium text-gray-800">{{ $produk->kategori->nama_kategori }}</span> •
                ...
                <div x-data="{ tab: 'detail' }" class="mt-6">
                    ...
                    <div class="border-b flex gap-6 text-gray-600 font-semibold pb-2">
                        <button @click="tab = 'detail'"
                            :class="tab === 'detail' ? 'border-b-2 border-primary text-primary' : ''">
                            Detail
                        </button>

                        <button @click="tab = 'review'"
                            :class="tab === 'review' ? 'border-b-2 border-primary text-primary' : ''">
                    ...
                    <div x-show="tab === 'review'" class="mt-4">
                        ...
                        @else
                            <p class="text-sm mt-4">
                                Silakan <a href="{{ route('login') }}" class="text-primary underline">login</a> untuk menulis
                                review.
                            </p>
                        @endauth
                    </div>
                ...
                <div class="flex gap-3 mt-6">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="btn btn-primary">Tambahkan Ke Keranjang</a>
                    @endauth

                    <a href="{{ route('cart.index') }}" class="btn btn-outline">Lihat Keranjang</a>
                </div>
            </div>
            ...
            @forelse($rekomendasi as $r)
                <div class="card bg-base-100 shadow hover:shadow-md transition">
                    <figure class="h-40 overflow-hidden relative">
                        ...
                        <img src="{{ $r->gambar_produk ? asset('storage/' . $r->gambar_produk) : asset('assets/images/no-image.svg') }}"
                    ...
                    <div class="card-body p-3">
                        ...
                        <p class="text-sm text-primary font-bold">Rp {{ number_format($r->harga, 0, ',', '.') }}</p>

                        <div class="card-actions justify-between mt-2">
                            <a href="{{ route('produk.show', $r->slug) }}" class="btn btn-xs bg-gray-200">Detail</a>

    ...
    <div class="max-w-6xl mx-auto px-4 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="relative w-full overflow-hidden rounded-lg shadow-md">
                ...
                {{-- Gambar --}}
                <img src="{{ $produk->gambar_produk ? asset('storage/' . $produk->gambar_produk) : asset('img/no-image.jpg') }}"
                    alt="{{ $produk->nama_produk }}"
                    class="w-full h-full object-cover opacity-0 transition-opacity duration-500" loading="lazy"
                        ...
                        @forelse($produk->reviews ?? [] as $rev)
                            <div class="border rounded-lg p-4 mb-3 bg-gray-50">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="font-semibold">{{ $rev->user->username }}</span>
                        ...
                        @auth
                            {{-- <form action="{{ route('produk.review.store', $produk->produk_id) }}" method="POST"
                                ...
                                <div>
                                    ...
                                    <textarea name="komentar" class="textarea textarea-bordered w-full" rows="3"
                                        required></textarea>
                                </div>
